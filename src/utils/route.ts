...
export const usePokemonIndexes = (pokemon: PokeResource[]): ListData => {
  ...
  let indexes = {
    prev: pokemon.length - 1,
    next: 1,
    current: currentIndex + 1,
  };

  if (currentIndex === -1) {
    indexes = { prev: pokemon.length - 1, next: 0, current: 0 };
  } else if (currentIndex === 0) {
    indexes = { prev: pokemon.length - 1, next: 1, current: currentIndex + 1 };
  } else if (currentIndex === pokemon.length - 1) {
    indexes = { prev: pokemon.length - 2, next: 0, current: currentIndex + 1 };
  } else {
    indexes = {
      prev: currentIndex - 1,
      next: currentIndex + 1,
